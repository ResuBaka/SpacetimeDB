    ...
    Self::Module: SpacetimeModule<DbUpdate = Self>,
{
    ...
    /// Apply this update using the connection's row-hook deduplication policy.
    ///
    /// The default preserves the behavior of bindings generated by older SDK versions.
    fn apply_to_client_cache_with_row_deduplication(
        &self,
        cache: &mut ClientCache<Self::Module>,
        _deduplicate_rows: bool,
    ) -> <Self::Module as SpacetimeModule>::AppliedDiff<'_> {
        self.apply_to_client_cache(cache)
    }

    /// Mark table updates represented in this message as initial subscription batches.
